/**
 * Raw YAML validation script using AJV directly
 * Validates YAML files against NEPA schema with minimal processing
 */
const fs = require('fs');
...
/**
 * Raw YAML validation using AJV directly
 */
async function validateYamlRaw() {
  console.log('🔍 Starting Raw YAML validation...');
  ...
  // Find YAML files
  const yamlFiles = findFiles(searchDirs, ['.yaml', '.yml'], ['schema', 'package', 'lock']);
...
module.exports = validateYamlRaw;
